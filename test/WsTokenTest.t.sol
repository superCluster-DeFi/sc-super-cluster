...
contract WsTokenTest is Test {
    ...
    function testWrapAndUnwrapFlow() public {
        ...
        vm.warp(block.timestamp + 1 days);
        sToken.updateAssetsUnderManagement(110 ether);
        sToken.rebase(110 ether);

        uint256 rate = wsToken.stTokenPerWsToken();
        assertApproxEqRel(rate, 1.1e18, 0.01e18, "Rate should reflect rebase");
        ...
        assertApproxEqRel(received, 110 ether, 0.01e18, "Unwrap yield mismatch");
    ...
    function testUnwrapToRecipient() public {
        ...
        vm.warp(block.timestamp + 1 days);
        sToken.updateAssetsUnderManagement(60 ether);
        sToken.rebase(60 ether);

        wsToken.unwrapTo(50 ether, user2);
        assertApproxEqAbs(sToken.balanceOf(user2), 60 ether, 2 ether, "Recipient unwrap mismatch");
    ...
    function testRateAndConversionFunctions() public {
        ...
        // Rebase by +50%
        vm.warp(block.timestamp + 1 days);
        sToken.updateAssetsUnderManagement(150 ether);
        ...
        assertApproxEqRel(stPerWs, 1.5e18, 0.01e18, "st/ws mismatch"); // 1.5x
